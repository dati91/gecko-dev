            ...
            if cache_path.exists() && cache_path.is_dir() {
                for entry in read_dir(cache_path).unwrap() {
                    ...
                    match deserialize_program_binary(&path) {
                        ...
                        _ => {
                            error!("Failed to desriralize program binary");
                        }
                    };
...
impl WrProgramCache {
    pub fn new(prof_path: &nsAString, workers: &Arc<ThreadPool>) -> Self {
        ...
        let _program_cache_observer = Box::new(WrProgramCacheObserver::new(Rc::clone(&disk_cache)));
        let program_cache = Rc::new(ProgramCache);//::new(Some(program_cache_observer));

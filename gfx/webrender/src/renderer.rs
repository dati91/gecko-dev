...
impl<B: hal::Backend> GpuGlyphRenderer<B> {
    fn new(_: &mut Device<Back::Backend>, _: &VAO, _: bool) -> Result<GpuGlyphRenderer<B>, RendererError> {
...
impl<B: hal::Backend> SourceTextureResolver<B> {
    fn new(device: &mut Device<Back::Backend>) -> SourceTextureResolver<B> {
...
pub struct Renderer//<B: hal::Backend>
{
    ...
    shaders: Shaders<Back::Backend>,

    pub gpu_glyph_renderer: GpuGlyphRenderer<Back::Backend>,
    ...
    #[cfg(feature = "debug_renderer")]
    debug: LazyInitializedDebugRenderer<Back::Backend>,
    ...
    prim_header_f_texture: VertexDataTexture<Back::Backend>,
    prim_header_i_texture: VertexDataTexture<Back::Backend>,
    transforms_texture: VertexDataTexture<Back::Backend>,
    render_task_texture: VertexDataTexture<Back::Backend>,
    gpu_cache_texture: CacheTexture<Back::Backend>,
    ...
    texture_resolver: SourceTextureResolver<Back::Backend>,
...
impl Renderer
{
    ...
    #[cfg(not(feature = "gleam"))]
    pub fn resize(&mut self, init: Option<DeviceInit<Back::Backend>>) -> DeviceUintSize {
        self.shaders.reset();
        ...
        let mut index = self.texture_resolver.render_target_pool
            ...
            .position(|texture| {
                ...
                if cfg!(not(feature = "gleam")) {
                    if texture.used_in_frame(frame_id) {
                        return false;
                    }
                }
